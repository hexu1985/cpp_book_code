/*************************************************************************
程序编号：7-1-6
程序功能说明：使用find_end寻找两容器之间是否含有相等或者
满足给定条件的子序列。
 *************************************************************************/
#include<vector>
#include<algorithm>
...
void main()
{
  ...
  result=find_end(vec1.begin(),vec1.end(),vec2.begin(),vec2.end());//数组1是否有子序列为数组2
  if(result!=vec1.end())
 	{
 ...
 cout<<"And the position is: "<<result-vec1.begin()<<endl;
  ...
  result=find_end(vec1.begin(),vec1.end(),vec3.begin(),vec3.end(),compare);// 数组1是否有子序列与数组3满足一定条件

  if(result!=vec1.end())
  {
	  cout<<"There is a sequence of elements in v1 that are\n";
	  cout<<" equivalent to those in v3 under the compare!"<<endl;
    cout<<"And the position is: "<<result-vec1.begin()<<endl;
  }
  else
  {
