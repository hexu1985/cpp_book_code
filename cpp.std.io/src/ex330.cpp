...
int main()
{
	...
	{    
		...
		cout << ufac.is(ctype_base::alpha,'?') << endl;
		cout << ufac.is_umlaut('?') << endl;
	// }}}
		// p. 331 {{{
		const ctype<char>& cfac = use_facet<ctype<char> >(loc);
		cout << cfac.is(ctype_base::alpha, '?') << endl;
		/*
		cout << cfac.is_umlaut('?')<< endl; // error: is_umlaut() is not accessible 
	...
	// p. 331 {{{
	umlaut<char> fac(1);
	cout << fac.is(ctype_base::alpha,'?') << endl;
	cout << fac.is_umlaut('?') << endl;
	// }}}
